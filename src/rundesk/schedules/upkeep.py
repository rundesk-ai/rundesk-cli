...
def _prompt(start: str, end: str, days: int) -> str:
    ...
    return (
        ...
        "self-improvement reference and finish its evidence and solution review. Start with no more "
        "than 50 messages or 20 turns, open at most three full turns, and record the reason before "
        "expanding either bound. Never exceed 100 messages, 40 turns, or five full turns; make no "
        "durable behavioral improvement from unresolved evidence at that ceiling. Use attention only when a "
        "specific owner action can resolve a required blocker. Select at most two repeated frictions, then "
        "choose only the relevant smallest durable, token- and cost-efficient route from memory or "
        "indexes, a verified agent-owned script, an existing or proposed skill, a materially better "
        "named specialist, or a provider-local research helper. When selected friction has an "
        "independent heavy research question and provider helpers are available, MUST launch at "
        "least one and synthesize its result. If a heavy research question exists but provider helpers "
        "are unavailable, record that unavailable route and continue only with bounded local evidence. "
        "If no selected friction contains such a question, record that fact without further helper "
        "analysis. Evaluate named "
        "agents only through public Rundesk surfaces, never their homes or memory. One owner "
        "correction cannot become durable memory, a standing rule, automation, a skill, or another "
        "durable behavior change unless it states an explicit durable preference or repeats earlier "
        "evidence. Apply and verify every safe authorized "
        "local improvement now; a retrospective or tracking update does not count as self-improvement "
        "by itself. Any script is incomplete until a post-edit "
        ...
        "is blocked, not done. A known operational failure that needs owner action requires the "
        "attention result; unrelated tracked work does not. Return exactly one short "
        "attention-first sentence and no audit detail: "
        "`Upkeep completed — improved <specific behavior>; no owner action is needed.`, "
        "`Upkeep completed — no durable change was justified; no owner action is needed.`, or "
