"""Somewhere to keep what a role run is, and the one callback that wakes its parent.

A named agent may hand one bounded task to an isolated role run — a fresh specialist
...
**What the run was admitted with.** Which role, which exact revision of it, which
...
**That its parent still has to be told.** A role run finishes while nobody is waiting
...
part of a role execution and never was; `role_run` is absent on it rather than
filled with anything, because a run that belongs to no role and one whose role was
...
def up(conn, home):
    # **The admission, not the execution.** One role run may be carried by more than
    ...
    # hanging the role's identity off whichever run happened to start first, loses the
    ...
    conn.execute("CREATE INDEX role_run_by_parent ON role_run(parent_run, n)")
    conn.execute("CREATE INDEX role_run_by_state ON role_run(state, n)")
    # **One row per role run, and the primary key is what says so.** A terminal outcome
    ...
    # another role, and a question that important is not answered from an environment
    ...
    conn.execute("ALTER TABLE run ADD COLUMN role_run TEXT")
    return []
