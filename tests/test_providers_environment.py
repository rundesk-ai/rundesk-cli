...
class ValuesThatCouldNotBeRead(support.Isolated):
    def test_oauth_clients_and_grants_are_never_exported_to_a_provider_turn(self):
        secrets.stated(oauth.STATE, '{"client_secret":"must-stay-inside-rundesk"}')
        self.assertNotIn(oauth.STATE, environment.owners_own())
