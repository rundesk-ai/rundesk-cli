...
class InstructionBuilder(unittest.TestCase):
    ...
    def test_a_profile_execution_is_never_told_the_named_agent_core_rules(self):
        """R-PRF-5 — the floor is what cannot be replaced, and it is small. A worker told
        it has a home, a memory and a Rundesk to operate goes looking for an identity it
        does not have."""
        built = instructions.for_profile(
            variables={"profile": "Development", "parent_agent": "elena",
                       "profile_run": "prf-1-aaaa", "target": "/projects/exporter",
                       "workspace": "/agents/elena/profile-runs/prf-1-aaaa/home/workspace"},
            rules="# Development\n\nRun the tests.\n",
        )
        ...
        self.assertIn("prf-1-aaaa", built)
        self.assertIn("/projects/exporter", built)

    def test_a_profiles_own_rules_reach_the_brain_exactly_as_they_were_written(self):
        """R-PRF-10 — a run resumes with byte-identical rules, and a substitution is a
        difference. Nothing is filled into what the owner wrote."""
        built = instructions.for_profile(
            variables={"parent_agent": "elena", "profile": "Development"},
            rules="Keep {agent_home} and {profile} literally.",
        )
        self.assertIn("Keep {agent_home} and {profile} literally.", built)

    def test_the_profile_layers_are_the_floor_then_the_rules_then_the_task(self):
        """R-PRF-5 — one stable order, so what a brain reads first is Rundesk's."""
        built = instructions.for_profile(
            variables={"parent_agent": "elena", "profile": "Development",
                       "profile_run": "prf-1-aaaa"},
            rules="# Mine",
        )
        self.assertLess(built.index("# Profile execution"), built.index("# Mine"))
