...
- `src/rundesk/config.py` — how this install is configured, as opposed to how any one agent is.
  ...
  never wrote without changing anything already stated, except carrying an explicitly renamed
  built-in identity to its replacement. `skills.granted` is the required baseline reconciled
  onto every new and existing agent and protected from revocation while configured.
...
- `src/templates/skills/` — **the skills this release ships.** Copied into the owner's library
  ...
  (R-AGT-30); `.renamed.json` states old package names whose attached grants and configured
  baseline entries move to a replacement, while unlisted Rundesk-owned packages are retired
  (R-AGT-35). `managing-rundesk` is how to operate rundesk, written for **an agent running
  inside it** — it was a document at the repository root that an agent had to be told to go and
  read, and the pointer named a path that existed on neither kind of install. As a skill it is
  handed to the agent instead. **Not every shipped skill reaches every agent**: the required
  baseline is `config.skills()["granted"]`, defaulting to the four that work rundesk itself and
  reconciled onto existing agents during updates and reinstalls (R-AGT-36).
