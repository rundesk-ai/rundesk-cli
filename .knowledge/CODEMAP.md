...
| File | Cases | Covers |
|---|---|---|
| `test_gateway.py` | 193 | `platform-gateway` — real processes, real signals, waits turned down |
...
| `test_answering.py` | 86 | `channel-messaging` — both edges are arguments, so a routing failure and a platform failure can never be confused |
