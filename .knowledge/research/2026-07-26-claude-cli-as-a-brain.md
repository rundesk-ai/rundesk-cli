...
| Be sent to mid-turn | Supported through the CLI's SDK control protocol: send an `interrupt` control request, wait for its acknowledgement, then send the replacement user message on the same process and session.[14] Measured offline against 2.1.220: the installed CLI accepts the SDK `initialize` handshake. Ordinary stream-JSON user messages alone do **not** steer; the CLI documents them as queued turns.[13] |
