...
- **GitHub calls an issue's type `issueType` in `gh issue ... --json`, not `type`.**
  Asking for `type` fails before returning any issues and prints the supported field list; use
