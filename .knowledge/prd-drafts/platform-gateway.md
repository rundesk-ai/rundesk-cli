...
| ✅ | R-GW-4 | Only one gateway runs at a time | `only one gateway of a name runs at a time`, `gateways of different names run alongside each other`, `a gateway claiming its own name twice is not a clash`, `one gateway stopping leaves the others running` |
...
| ✅ | R-GW-16 | A gateway ends whatever the last gateway of its name left running | `taking a name ends what the last gateway of it was running`, `what is in flight is written down as it happens`, `work recorded by a gateway that has since gone is left alone` |
| ✅ | R-GW-17 | The gateway says so when it goes with work it could not end | `stopping does not wait past the time it is allowed` |
| ✅ | R-GW-18 | What a gateway wrote about what happened outlives the gateway | `what a gateway wrote outlives the gateway`, `a gateway records coming up and going down`, `work that ended badly is recorded with its last words`, `a gateway that refused to start says why in writing` |
...
- Whether ending recorded work by its process group is safe enough, given that a number can in principle
  be reissued to something unrelated once the group that held it has gone.
