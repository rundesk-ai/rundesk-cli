...
| ✅ | R-GW-4 | Only one gateway of a given name runs at a time | `only one gateway of a name runs at a time`, `gateways of different names run alongside each other`, `a gateway claiming its own name twice is not a clash`, `one gateway stopping leaves the others running`, `a name is not left held by a claim that did not finish` |
...
| ✅ | R-GW-12 | Stopping the gateway leaves nothing of it behind to be found by the next start | `a gateway asked to stop goes`, `stopping leaves nothing for the next start to find`, `the name a gateway gave back can be taken again`, `it is beside the install rather than inside the source`, `where it keeps things can be said` |
