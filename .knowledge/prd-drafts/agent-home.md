...
configured, what each remembers, and where each works by default. They are not separated by the
operating system, so a provider's own file and shell tools may still reach anything their owner can.
