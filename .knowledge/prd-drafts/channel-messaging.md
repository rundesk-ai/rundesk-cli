...
| ❌ | R-CH-11 | A channel carries a turn through to an answer whatever its platform cannot do | — |
| ❌ | R-CH-12 | A channel that cannot connect refuses to start and names what is missing | — |
| ❌ | R-CH-13 | A channel leaves nothing running once the turn it belonged to has ended | — |
| ❌ | R-CH-14 | A failure to deliver on a channel does not end the turn it was reporting | — |
| ❌ | R-CH-15 | Raw tool arguments and results do not leave the machine unless a rule allows them | — |
| ❌ | R-CH-16 | A conversation's session is found again after the gateway holding it restarted | — |
| ❌ | R-CH-17 | Work a channel dispatched is findable afterwards by the run it became | — |
