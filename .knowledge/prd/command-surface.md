...
| ✅ | R-CMD-11 | An owner can ask what this install is configured with, which of it defaulted, and what they wrote that nothing reads | `what an install is configured with is answerable`, `a value that was stated is told apart from one that defaulted`, `a key nothing reads is said rather than left out`, `an unreadable configuration is refused rather than reported as defaults` |
