...
| ✅ | R-DIS-32 | An inbound Discord reply carries its referenced message into Rundesk's shared reply context | `on message reports the reply on the arrived record`, `a resolved reply carries the parent identity author and body`, `a deleted or unfetched parent still carries its identity`, `a non reply reference is not presented as a reply`, `a message without a reference has no reply context` |
