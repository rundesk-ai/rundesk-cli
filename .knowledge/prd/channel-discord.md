...
| ✅ | R-DIS-32 | A connected Discord bot identifies itself as Rundesk followed by its agent's human-readable name | `a discord gateway is named for its agent`, `a discord gateway name stays inside the platform limit`, `discords reserved word cannot make an agent name fail`, `connecting renames the bot once`, `an already named bot is not edited` |
