...
| ✅ | R-DIS-32 | A completed Discord answer begins its completion line with the provider that ran the turn | `the whole footer an owner reads is the size what was written and the clock`, `provider and elapsed time are shown when usage was not reported` |
