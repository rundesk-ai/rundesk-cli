...
| ✅ | R-CH-22 | An owner writes what their agent is told about a situation, and it is checked when they write it | `what an owner has an agent told composes what is always true with what fits`, `an owner who wrote nothing is still told where the agent is`, `a situation that does not exist is refused when it is written`, `a name that cannot be filled in is refused when it is written`, `a brace an owner wrote for its own sake is left alone`, `what an agent is told is bounded however it was composed`, `what an agent is told is written and read back`, `one situation being written does not drop another`, `a name that cannot be filled in is refused before it is written`, `telling a channel that is not there says so`, `a surface is named rather than located` |
