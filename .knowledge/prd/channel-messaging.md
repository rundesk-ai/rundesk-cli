...
| ✅ | R-CH-28 | An inbound reply separates its context, identifies the earlier conversation message, and bounds quoted text by REPLY_TEXT_MOST with an explicit truncation marker | `a reply reference crosses the seam with bounded context`, `a reply tells the brain which message the follow up is for`, `an ordinary message has no empty reply context` |
