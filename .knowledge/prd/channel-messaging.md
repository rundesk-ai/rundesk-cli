...
| ❌ | R-CH-11 | A channel leaves nothing running once the turn it belonged to has ended | src/rundesk/gateway.py:1300 — inherited from what ends a program at all; no test of its own here; .knowledge/scripts/probe-discord says what to do and what to look for |
...
| ✅ | R-CH-13 | Raw tool arguments and results do not leave the machine unless a rule allows them | `raw tool arguments and results do not leave the machine`, `a field nobody here knows stays here`, `a safe helper name leaves but unrelated tool fields do not`, `a summary too long to show is bounded rather than dropped` |
...
| ✅ | R-CH-22 | An owner writes what their agent is told about where it is, and it is checked when they write it | `what an owner has an agent told is one piece of text`, `a place arrives in pieces as well as in words`, `a piece of a place is a strangers words and is treated as such`, `a channel scopes itself so two surfaces are two channels`, `an owner who wrote nothing anywhere is still told where the agent is`, `a channel that says nothing falls to what the agent says`, `what this channel says still wins`, `what this channel says still wins over the agents`, `what is not words at all is refused when it is written`, `a name that cannot be filled in is refused when it is written`, `a brace an owner wrote for its own sake is left alone`, `what an agent is told is bounded however it was composed`, `a surface is named rather than located`, `what an agent is told is written and read back`, `writing it again replaces it and empty takes it off`, `a name that cannot be filled in is refused before it is written`, `telling a channel that is not there says so` |
