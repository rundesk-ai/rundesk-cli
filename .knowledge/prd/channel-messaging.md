...
| ❌ | R-CH-11 | A channel leaves nothing running once the turn it belonged to has ended | src/rundesk_cli/gateway.py:1300 — inherited from what ends a program at all; no test of its own here; .knowledge/scripts/probe-discord says what to do and what to look for |
...
| ❌ | R-CH-16 | A person can restart the agent answering them, from the channel they are on | src/rundesk_cli/answering.py:166 — proved by hand against a real gateway; a case needs one the machine will bring back; .knowledge/scripts/probe-discord says what to do and what to look for |
