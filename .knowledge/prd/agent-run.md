...
| ✅ | R-RUN-24 | A resumed turn that reported no cost, classified nothing and answered nobody is asked once more on a fresh session, in its own account | `a resumed turn that never ran is asked again on a fresh session`, `being asked again is in the runs own account`, `a second silence is the answer and nothing is asked a third time`, `a turn that was not resumed is not asked again`, `a turn that spent anything at all read something`, `a brain that measured nothing is not a brain that did nothing`, `a brain that said why it stopped stopped for a reason of its own`, `a turn whose brain never said it ended is not asked again`, `a program that did not finish well failed for its own reason` |
