...
| ✅ | R-RUN-24 | A resumed turn answering nobody, classifying nothing and reporting no cost is asked once more on a fresh session, unless it continues rundesk's own work | `a resumed turn that never ran is asked again on a fresh session`, `the conversation carries on from the fresh session not the stale one`, `being asked again is in the runs own account`, `a second silence is the answer and nothing is asked a third time`, `a turn that was not resumed is not asked again`, `a turn rundesk asked for itself is not asked again`, `a recovery turn is refused rather than begun again`, `a turn that spent anything at all read something`, `a brain that measured nothing is not a brain that did nothing`, `a brain that said why it stopped stopped for a reason of its own`, `a turn whose brain never said it ended is not asked again`, `a program that did not finish well failed for its own reason` |
