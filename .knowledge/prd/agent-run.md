...
| ✅ | R-RUN-23 | What brains printed longer ago than the kept window is swept, and every account still reads | `what a brain printed longer ago than the window is swept`, `sweeping what a brain printed leaves every account readable`, `sweeping where nothing was ever printed is ordinary`, `a run that printed nothing still has what went wrong swept`, `a run that printed nothing recently is still inside the window` |
