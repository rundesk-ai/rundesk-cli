...
| ❌ | R-CAD-6 | An agent's channels are held open by the gateway that runs it | src/rundesk/gateway.py — Gateway._hold_channels, Gateway._hold_one — proved by hand against a real Discord gateway, not by a test: the case needs a gateway serving a channel for real |
| ❌ | R-CAD-7 | A channel that drops its connection returns without a turn noticing | src/rundesk/gateway.py — Gateway._hold_one — a turn surviving a drop is proved, the adapter coming back on its own is not |
...
| ✅ | R-CAD-16 | An adapter is told which place to say something in, and resolves that word itself | `a name with its hash and without are one room`, `a room is found however it was capitalised`, `a different room is not this one`, `what an owner typed is taken as they typed it`, `a dm place that is a user id opens that persons dm`, `a dm channel id still resolves as a channel`, `a user id is not opened as a dm from a room channel`, `a dm place refuses a user who is not allowed`, `a room that is simply missing is not reported as a refusal` |
...
| ✅ | R-CAD-19 | Who may reach an agent through a channel can be shown and changed on the channel that already exists, and never reduced to nobody | `who is allowed is shown one to a line`, `somebody is added to a channel that already exists`, `somebody is taken off a channel that already exists`, `one person is replaced by another in one change`, `the last person allowed cannot be taken off`, `taking off somebody who was never allowed is refused`, `a user id with nothing in it allows nobody`, `a change that changes nothing says so`, `a channel that is not there is told from one that is`, `adding somebody says the surface hears of it when it next starts`, `who a surface allows is changed without rewriting the rest of it`, `one person is replaced by another under one hold`, `a surface cannot be changed down to nobody`, `taking off somebody a surface never allowed is refused`, `a user id with nothing in it allows and removes nobody`, `changing who a surface that is not there allows is refused` |
| ✅ | R-CAD-20 | A record handed to an adapter names its conversation with the platform's own identifier, never one internal to Rundesk | `what a schedule hands a surface is the platforms own conversation`, `a conversation that has gone is nowhere to report`, `a conversation that is no snowflake is nowhere rather than a crash`, `a conversation that is a snowflake still resolves` |
| ✅ | R-CAD-21 | A value this install keeps is never given to an adapter under the name that adapter reads its own credential from (R-SEC-29) | `a surface is never given the value it reads its own credential from` |
