...
| ❌ | R-CAD-6 | An agent's channels are held open by the gateway that runs it | src/rundesk_cli/gateway.py:1276 — proved by hand against a real Discord gateway, not by a test: the case needs a gateway serving a channel for real |
| ❌ | R-CAD-7 | A channel that drops its connection returns without a turn noticing | src/rundesk_cli/gateway.py:1300 — a turn surviving a drop is proved, the adapter coming back on its own is not |
...
| ✅ | R-CAD-9 | Adding a channel proves it can connect before anything about it is written down | `an adapter says whether it can reach what it was pointed at`, `an adapter that cannot reach its platform says why`, `an adapter that answers with nonsense has proved nothing`, `an adapter that says nothing is reported by what it said went wrong`, `adding a channel that cannot connect writes nothing and says why`, `adding a channel that works writes what the adapter asked to keep` |
...
| ✅ | R-CAD-11 | A credential a channel needs is read from where the owner keeps it, never from a command | `an adapter names a credential rather than giving one`, `the one credential an adapter named is the only one it gets`, `a credential that is not set is not invented`, `the credential is read from the environment and never an argument`, `a supervised gateway finds its token without a shell` |
