...
| ✅ | R-RM-7 | Removing rundesk takes what the install put there for it | `removing rundesk takes what was installed for it`, `uninstall takes a blocked renamed builtin but keeps the owner collision` |
...
| ✅ | R-RM-12 | Removing the program cannot reach what the owner keeps, rather than remembering not to | `removing the program cannot reach what the owner keeps`, `purging takes what the owner keeps as well`, `removing rundesk leaves a checkout where it stands`, `a downloaded install puts the program in its own directory`, `a redirected uninstall cannot take another installs data` |
| ✅ | R-RM-13 | Removing rundesk keeps the templates an owner wrote unless removal is asked to take them | `removing rundesk keeps the templates an owner wrote`, `purging takes the templates an owner wrote as well` |
| ✅ | R-RM-14 | Copies of what the owner keeps survive removal, including one asked to take the data | `removing rundesk keeps the copies it took`, `purging keeps the copies it took`, `removing rundesk says the copies are kept rather than offering to purge them`, `taking the older program away keeps copies standing beside it` |
| ✅ | R-RM-15 | Removing one install leaves the jobs another install wrote, says which, and finishes | `removing one install leaves the shared jobs another install wrote`, `leaving another installs jobs alone does not end the removal`, `removing the install that wrote the shared jobs takes them`, `shared jobs that were never written are not a refusal` |
| ✅ | R-RM-16 | Removal keeps the values every program is given, and a purge asked to take the owner's data takes them (R-SEC-26) | `removing rundesk keeps the values every program was given unless asked`, `a purge refuses a secrets directory it did not write` |
