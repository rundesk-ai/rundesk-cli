...
| ✅ | R-MIG-7 | An owner is told which agent could not be moved, which step failed, and what it had reached | `which step failed and the version it reached are both named`, `an agent whose records cannot be moved names that agent`, `what went wrong is left where rundesk logs will find it` |
