...
| ✅ | R-SLK-44 | Slack needs two credentials, and both are named rather than handed over | `both credentials are named and only named`, `no credential is ever written into the settings`, `an option it does not understand is refused by name`, `a considered refusal still exits zero` |
