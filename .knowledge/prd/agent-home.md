...
| ✅ | R-AGT-36 | Every new and existing agent receives Rundesk-required skills plus the baseline selected in install configuration | `a new agent is given what the configuration says`, `an existing agent is reconciled to the required baseline`, `laying down also reconciles every existing agent`, `an agent is made with the skills written into a new configuration`, `an owner who wants no optional skills still gets rundesk stewardship`, `an empty optional list still keeps the rundesk operating baseline`, `every required skill is part of the release`, `a configuration that cannot be read refuses to make the agent`, `a diagnosis judges an agent against what is configured`, `rundesk issue guidance makes every agent a bounded platform steward` |
