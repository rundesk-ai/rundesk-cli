...
| ✅ | R-AGT-36 | Every new agent receives every skill required by the install configuration | `a new agent is given what the configuration says`, `an agent is made with the skills written into a new configuration`, `an owner who wants no skills granted gets none`, `a configuration that cannot be read refuses to make the agent`, `a diagnosis judges an agent against what is configured` |
