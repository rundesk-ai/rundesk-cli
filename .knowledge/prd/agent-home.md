...
| ✅ | R-AGT-40 | New agent templates name their agent with `{{agent}}`, while existing owner templates using `{{name}}` keep working | `an owner template uses the agent placeholder`, `a legacy name placeholder still names the agent` |
