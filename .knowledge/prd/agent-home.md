...
| ✅ | R-AGT-35 | A release retires removed built-ins, migrates explicitly renamed grants and required baselines after their replacement is present, and preserves owner-created skills and entries | `a removed built in is revoked and retired`, `a renamed built in moves its grant before retiring the old name`, `a rename blocked by an owner entry leaves the working grant`, `a new library name owned by the owner does not claim the old one`, `a configured renamed built in follows its replacement` |
