...
| ✅ | R-AGT-35 | A release retires built-ins it no longer ships, migrates explicitly renamed grants only after their replacement is present, and leaves owner-created skills and entries untouched | `a removed built in is revoked and retired`, `a renamed built in moves its grant before retiring the old name`, `a rename blocked by an owner entry leaves the working grant`, `a new library name owned by the owner does not claim the old one` |
