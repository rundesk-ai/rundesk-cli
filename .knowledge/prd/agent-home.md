...
| ✅ | R-AGT-9 | An agent the machine keeps running resolves the home the command that made it resolved | `the job carries the directories it was given rather than its own`, `a verb asks where the agent it names keeps things`, `where an agent keeps things is its own` |
