...
| ❌ | R-AGT-10 | An agent loads what stands in its own home rather than what its owner keeps | src/rundesk_cli/turn.py — codex was asked, standing there, a question only the scaffolded AGENTS.md answers, and answered it with no tools; a case needs a real provider and a credential, which the suite has neither of |
