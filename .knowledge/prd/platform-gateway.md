...
| ✅ | R-GW-13 | Every gateway an owner starts, inspects or stops is reached through the command line | `serving runs the gateway of the name given`, `starting hands it over`, `stopping a named gateway stops that one`, `cycling a gateway stops it and starts it again`, `only our jobs are listed`, `someone elses job is never removed`, `someone elses job is never stopped or started`, `someone elses job is never handed to the machine as ours`, `handing over a name belonging to something else is refused`, `cycling waits for the old one to actually go`, `cycling says so rather than starting one that never stopped` |
