...
| ✅ | R-GW-31 | A gateway is taken away completely, its job included, through the command line | `removing a gateway takes its job and what was kept for it`, `stopping with remove does both in one step`, `removing a gateway that was never there says so rather than failing`, `forgetting a gateway that was never there takes nothing and says so`, `forgetting a gateway leaves every other gateway alone`, `remove without a name answers in our words`, `removing every gateway at once is not something stop will guess` |
| ✅ | R-GW-32 | Removing a gateway that anything still holds takes nothing at all | `removing a gateway that is running removes nothing`, `removing a gateway the machine will not let go of removes nothing`, `stopping with remove removes nothing when it did not stop`, `forgetting a gateway never unlinks a lock something else is holding`, `removing a gateway belonging to another install is refused` |
| ✅ | R-GW-33 | What a gateway wrote is kept when it is removed unless removal is asked to take it | `removing a gateway keeps what it wrote unless purge is asked for`, `forgetting a gateway keeps what it wrote and what was scheduled`, `forgetting a gateway with its history takes all of it` |
