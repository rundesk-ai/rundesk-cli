...
| ✅ | R-INS-13 | Everything rundesk's install is made of lives in one directory under the person's home | `the install lives under the persons home`, `an install writes nothing outside the places it says` |
...
| ✅ | R-INS-15 | An install with no copy of the source takes the newest published version, not the newest work | `an install without a checkout takes the newest release not the branch`, `a release lookup that fails falls back instead of dying silently` |
