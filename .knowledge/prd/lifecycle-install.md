...
|:--:|---|---|---|
| ✅ | R-INS-1 | A machine with none of rundesk on it installs it with one instruction | `the one instruction names the same thing everywhere it is given`, `the one instruction points at the repository rundesk updates from`, `a release serves one counted program archive`, .github/workflows/build.yml:one-instruction — the published release installed from an empty directory |
...
| ✅ | R-INS-14 | An install changes only its declared paths, command placement, and the fresh-install star R-INS-21 permits | `an install does not change the path it only says so`, `an install refuses a directory too important to be one programs`, `an install refuses to replace a checkout it did not create`, `an install refuses to replace a command of the same name` |
...
| ✅ | R-INS-18 | An install elsewhere names its jobs apart, so what it hands the machine and takes back is only ever its own | `a second install names every job apart from the first`, `a second install asks the machine only for its own registration`, `a gateway of another prefix is not this installs`, `nothing said is the prefix rundesk ships`, `a prefix that could escape the jobs directory is refused`, `a prefix inside the default namespace is refused`, `a job prefix that cannot be one stops the command`, `every job carries the name this install gave it`, `a job of this install asks the machine only for this installs`, `a program is told what this install calls its jobs`, `nothing said writes the job that shipped` |
| ✅ | R-INS-19 | An install writes every effective install-wide value into its configuration file | `installing writes the configuration an owner can read`, `installing writes every value that governs the install`, `installing again changes nothing an owner configured` |
| ✅ | R-INS-20 | A remote installation from the counted-delivery release onward contributes once to Rundesk's public installs count | `a remote install downloads the counted release asset once`, `a counted release never falls back to an uncounted download` |
| ✅ | R-INS-21 | A fresh install with authenticated GitHub CLI stars Rundesk once and says it helped support the community | `a fresh install with authenticated github cli stars rundesk`, `installing again makes no second star request`, `an install without github cli still succeeds`, `an install without github authentication does not star`, `a failed star request does not fail the install` |
