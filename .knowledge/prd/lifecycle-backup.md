...
| ✅ | R-BKP-13 | How many copies are kept and when one is taken are the owner's to state | `an install reads backup values from its configuration`, `how many backups are kept is the owners to state`, `a time of day is read the way a person writes one` |
| ✅ | R-BKP-14 | Configuration that cannot be understood is refused rather than replaced by a default | `configuration that cannot be understood is refused rather than ignored`, `a count that would keep nothing is refused`, `a time of day that is not one is refused` |
...
| ✅ | R-BKP-22 | Copies beyond the stated number are taken away once a newer one exists | `copies beyond the stated number are taken away`, `a copy whose identity cannot be read is never the one removed` |
| ✅ | R-BKP-23 | The newest copy is never taken away by the count | `the only copy there is is never taken away by the count`, `a count already exceeded still leaves the newest one` |
...
| ✅ | R-BKP-31 | A reading of the backup directory that fails outright says what it failed with, rather than reporting the bound as exceeded | `a backup directory that refuses is named by what it refused with`, `a prune that fails is reported by its cause rather than the bound` |
