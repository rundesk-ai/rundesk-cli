...
- Catalog changes cannot replace owner-authored skills or change a running agent's grants.
...
| ✅ | R-CAT-5 | A catalog never replaces or removes owner-authored skill content | `owner authored skill with same name refuses install`, `confirming a colliding repository fails with the skill name`, `an install refuses a default catalog name owned by the user`, `removing takes only the catalog and its library links`, `removing without yes previews and changes nothing`, `updating leaves same named custom agent content alone` |
...
| ✅ | R-CAT-8 | A failed catalog change leaves the previously working version available | `a failed update leaves the working version`, `a failure after activation rolls back files links and provenance`, `failed adoption restores the retired built in`, `a failed removal restores catalog links before grants` |
| ✅ | R-CAT-9 | Updating or removing a catalog revokes removed skills from stopped agents and refuses while an affected agent is running | `an update cannot remove a granted skill`, `removing refuses while one of its skills is granted`, `grant changes share one install wide lock`, `granting uses the same lock as catalog retirement`, `updating revokes a removed skill from a stopped agent`, `updating refuses to remove a skill from a running agent`, `a failed catalog update restores the grant it retired`, `grant rollback attempts every agent after one restore fails`, `removing a catalog revokes its skills from stopped agents`, `uninstalling the seeded catalog retires stopped agent grants` |
