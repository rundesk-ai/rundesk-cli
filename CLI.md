...
# reaching it
rundesk ask [--provider <provider>] [--model <model>] [--set <key=value>] [--conversation <conversation>] [--fresh] [--read-only] [--steer] [--label <text>] [--posture read|work] [--instructions <text>] <agent> <prompt>   one turn, streamed to this terminal
rundesk ask [--provider <provider>] [--model <model>] [--set <key=value>] [--conversation <conversation>] [--fresh] [--read-only] [--steer] [--label <text>] [--posture read|work] [--instructions <text>] <agent> <prompt>   with standing instructions, told apart from the prompt
...
#   cole answers once, with its own home, memory, skills and brain, and ava reviews that answer before anybody else hears about it
# narrower than the turn handing it over; it can never be wider — and with no task argument it is read from standard input, which keeps a long one out of `ps`
...
--label <text>                  only when one agent asks another: a short safe name for the task, shown where other people can read it
--model <model>                 which model, in that brain's own words
--most <n>                      how many to show, newest first (default: 20)
--posture read|work             only when one agent asks another: how far the answering agent may reach — never wider than the turn handing the work over
