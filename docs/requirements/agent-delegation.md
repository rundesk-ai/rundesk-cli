...
| ✅ | R-DEL-27 | Each agent's outbound delegation scope is unrestricted by default, may be an exact allowlist or empty, filters every team shown to that agent, omits the complete named-agent instruction layer when empty, and refuses a direct handoff outside the same scope before writing anything; new and enabled delegators hold the bundled delegation workflow while an empty scope revokes it, and update reconciles older agents to the same rule; target removal prunes explicit allowlists so name recreation inherits no old authority; scope changes, revocation, and admission serialize under the install lock; NULL stays unrestricted and inbound delegation is unchanged | `test_a_made_agent_has_run_it_and_may_delegate_to_any_agent_by_default`, `test_a_new_agent_is_given_the_delegation_skill_its_default_scope_needs`, `test_delegate_to_none_removes_the_delegation_skill`, `test_delegate_to_any_restores_the_delegation_skill`, `test_the_sweep_aligns_the_delegation_skill_with_each_agents_scope`, `test_an_unlisted_target_is_refused_before_either_delegation_write`, `test_a_completed_revocation_cannot_be_followed_by_stale_authority_admission`, `test_an_inbound_only_preview_omits_the_complete_named_agent_layer` |
...
| ✅ | R-DEL-30 | An authorized channel query stays answerable when a target agent's own records cannot be read — not there, half-written, or impossible to open: every relevant named-agent item is still listed in each live state with that task identity reported as explicitly unavailable rather than guessed, no other named-agent or provider-local item is hidden, and a failure to read the asking agent's own records is still a failure | `test_an_unreadable_target_brief_keeps_named_and_local_work_listed`, `test_every_live_named_state_stays_listed_when_the_target_brief_cannot_be_read`, `test_a_target_store_that_is_missing_or_unopenable_reads_as_identity_unavailable`, `test_the_asking_agents_own_unreadable_records_are_still_a_failure` |
