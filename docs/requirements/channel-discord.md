...
| R-DIS-26 | A gateway returning from update maintenance names the installed version now listening and links that release. | `tests/test_gateway_maintenance.py`; `TheChannelsItHosts.test_a_gateway_returning_from_an_update_names_and_links_the_installed_release`; real-platform presentation check. |
| R-DIS-27 | An ordinary gateway startup adds no update wording and no release link. | `TheChannelsItHosts.test_a_gateway_that_came_up_says_so_through_the_channel_that_is_told_things`; compare with the update-maintenance case. |
