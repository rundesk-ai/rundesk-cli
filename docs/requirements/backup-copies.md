...
| ✅ | R-BKP-2 | The archive is written forward without seeking on its destination, under a private `.incoming` name, verified there and renamed into place only when whole, so a destination that rejects a seek still receives a verified copy and a copy that did not finish is never named like one that did. Staging litter that will not delete after the copy is named costs a sentence and never the copy. | `test_a_cloud_location_that_rejects_zip_seeks_receives_a_verified_copy`, `test_a_copy_that_could_not_be_renamed_into_place_leaves_nothing_named`, `test_a_verified_copy_survives_its_staging_litter_and_says_so` |
