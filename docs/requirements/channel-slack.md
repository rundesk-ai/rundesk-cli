...
| ✅ | R-SLK-9 | An unallowed sender or place costs no platform call, produces no channel record and receives no response. A fixed, content-free diagnostic may name the boundary in the adapter's stderr log without carrying message text or identifiers. | `test_a_stranger_costs_nothing_and_is_told_nothing`, `test_a_place_it_does_not_allow_costs_nothing`, `test_an_ignored_event_logs_only_its_fixed_boundary`, `test_a_forged_display_name_never_reaches_the_decision` |
...
| ✅ | R-SLK-28 | The adapter logs each fixed event boundary at most once per run, without message text, member, channel, workspace, or credential values. | `test_an_ignored_event_logs_only_its_fixed_boundary`, `test_each_diagnostic_boundary_is_logged_once`, `test_an_allowed_event_logs_that_it_reached_the_channel` |
...
| ✅ | R-SLK-30 | The shipped manifest marks the Socket Mode bot always online and does not claim that Slack presence proves gateway health. | `test_the_manifest_keeps_the_bot_visible_without_calling_presence_health` |
