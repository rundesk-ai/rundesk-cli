...
| R-CAD-11 | A credential an adapter needs is entered outside the command line, kept outside backups, and supplied only under the name the adapter declared. | Inspect argv, persisted channel settings, backup contents, and the stand-in adapter environment. |
