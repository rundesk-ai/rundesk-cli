...
| ✅ | R-INS-33 | Every turn may end in exactly one of three states — a verified outcome, a named blocker carrying its next action, or unfinished work with an actual Rundesk resumption — and work waiting on that resumption is never reported as complete | `test_every_turn_cannot_end_without_delivery_or_a_continuation_path`, `test_every_turn_stops_when_the_requested_result_and_proof_are_complete` |
...
| ✅ | R-INS-35 | Every turn separates durable state from resumption: history and saved state preserve context but do not resume work; work spanning turns keeps its current state, evidence, and next action in a project-owned durable artifact or active `tasks/` brief rather than changing-status memory; unfinished work with neither a blocker nor an actual resumption stays in the current turn; an enabled `--ask` self-schedule establishes the resumption path without another infrastructure check; and only that path gets a two-sentence final with the verified current outcome followed by the scheduled result and time, adding a third only for a current material blocker | `test_long_running_work_keeps_state_and_a_real_resumption_path` |
