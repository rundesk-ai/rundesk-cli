...
>
> **What is still true.** The whole of `What they do` — the survey of how six migration tools order
> and record steps, the SQLite documentation quotations, and the probe of SQLite 3.51.0 through the
> macOS standard-library binding that measured transactional DDL, WAL persistence, `busy_timeout`
> defaults and `BEGIN IMMEDIATE` contention. **What is not: the verdict.** It concludes that
> `PRAGMA user_version` is the right home for the schema number and that a `rundesk.json` holds a
> layout version beside it. This build does neither — it records one row per applied step and derives
> every location from one root — so read the verdict as the previous build's decision and the
> reporting above it as the evidence. The reasoning that survives is in the report that carried this
> page across.
