...
>
> **What is still true.** All of it. It reports four independent bodies of vendor guidance about the
> `SKILL.md` format and cites nothing of the previous build's architecture, so nothing in it was
> invalidated by the rewrite. The size and frontmatter limits are the specification's and are the
