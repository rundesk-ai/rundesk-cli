...
>
> **What is still true.** Everything about `codex-cli 0.145.0` — including the cumulative-usage trap,
> which is the single most transferable finding in the file. **What is not.** `app-server` was marked
> experimental at that version, so the protocol half needs re-probing before anything is built on it,
> and the references to a shipped adapter and to `RUNDESK_*` variables are the previous build's.
