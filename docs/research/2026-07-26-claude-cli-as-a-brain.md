...
>
> **What is still true.** All of it is about a vendor CLI rather than about rundesk, so the rewrite
> did not touch it — but it is true of `claude 2.1.219` and `2.1.220` on macOS 25.5.0 in July 2026
> and nothing has been re-driven since. **What is not.** Its references to `RUNDESK_PREFACE`, to a
> shipped adapter and to a `provider-adapter` requirement list are the previous build's; the contract
> those belong to is distilled in [`the-adapter-contracts.md`](the-adapter-contracts.md), and this
> build has not written its provider layer yet.
...
Measured, on the golden: 13 `tool_use` blocks with ids of the form `toolu_011WKuk…`,
