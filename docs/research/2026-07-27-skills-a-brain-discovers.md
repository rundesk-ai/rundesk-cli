...
>
> **What is still true.** The measured discovery table, the symlink behaviour, and the conclusion that
> the scope is the directory a turn stands in rather than any configuration. **Read the discovery
> table against what this build plans**: it measured that a bare `skills/` directory is discovered by
> no brain at all, that `claude` reads only `.claude/skills`, and that `codex` does not read
> `.claude/skills`. **What is not.** `RUNDESK_PROVIDER_HOME` and the requirement ids are the previous
> build's vocabulary.
