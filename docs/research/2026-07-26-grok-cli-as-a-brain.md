...
>
> **What is still true.** Everything about `grok 0.2.111` and `0.2.112`, and in particular the two
> findings that generalise: a vendor flag that is accepted and enforces nothing, and a CLI that
> answers from conversations it was never handed. **What is not.** Its `RUNDESK_*` and shipped-adapter
> references are the previous build's, and the ACP surface is the fastest-moving thing in the file.
...
| Say which conversation this was | Measured: `end.sessionId`. The golden was captured with no `--session-id` passed and came back with `019f95ad-…`, so the CLI mints one when the caller does not.[1][10] |
