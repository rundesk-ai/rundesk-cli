...
`delegate`, `memory`, `rules`, `identity` — or absent. Never render your own word
for one; a channel that recognised a vendor's tool names would carry that vocabulary forever.

**The last three are the agent rewriting a file it lives by**, and each wants its own mark.
Folding them back into `edit` throws away the only part a reader wanted: `MEMORY.md`,
`AGENTS.md` and `SOUL.md` are what an agent *is* between turns.
